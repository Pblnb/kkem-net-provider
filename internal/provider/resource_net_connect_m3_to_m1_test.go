...
func newInvalidM3ToM1ResourceState(t *testing.T) tfsdk.State {
	t.Helper()
	state := tfsdk.State{Schema: m3ToM1ResourceSchema(t)}
...
func newM3ToM1ResourceModel() netConnectM3ToM1ResourceModel {
	return netConnectM3ToM1ResourceModel{
		...
		M3VpcepEndpointSubnetId: types.StringValue(testM3SubnetId),
...
func newM3ToM1ResourceStateModel() netConnectM3ToM1ResourceModel {
	return netConnectM3ToM1ResourceModel{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.invalidState {
				...
				req.State = newM3ToM1ResourceStateWithModel(t, newM3ToM1ResourceStateModel())
			}

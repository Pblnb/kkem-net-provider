...
3. **保存新工具**：如果写了有复用价值的分析脚本，将其保存为 `scripts/agents/design-<what>.sh`（或 `.py`）
...
每个函数只做一件事。一个函数超过 80 行通常意味着违反了 KISS（`go-structure.json` 提供数据，design-agent 分析根因）。
...
**检查点**：
- 资源打开后是否立即跟 `defer` 关闭
- 多层嵌套 if 是否可以用 early return 展平（结合 `go-structure.json` 的 `over_4_nesting`）

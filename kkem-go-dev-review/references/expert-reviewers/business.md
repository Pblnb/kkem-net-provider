...
- 变更代码内容（git diff，由 Reviewer Coordinator 以文本形式传入）
- 完整文件内容（用 `Read` 工具读取变更文件的完整版本，不仅看 diff）
- commit message / PR 描述（如果 Reviewer Coordinator 提供了的话）
...
3. **保存新工具**：如果写了有复用价值的分析脚本，将其保存为 `scripts/agents/business-<what>.sh`（或 `.py`）

...
- `diagnostics.json`（来自 run-go-tools.sh）**——本 Agent 必须读取并综合此文件数据**
- `rule-hits.json` 中属于本 Agent 的命中项（来自 Tier 2 scan-rules.sh）
- 变更代码内容（由 Reviewer Coordinator 以文本形式传入，无需自行执行 git 命令）
...
3. **保存新工具**：如果写了有复用价值的分析脚本，将其保存为 `scripts/agents/quality-<what>.sh`（或 `.py`）
...
同时读取 `/tmp/go-structure.json`（来自 `analyze-go.sh`）：
- `functions_over_80` → 结合函数职责判断是否需要拆分
- `nesting_violations` → 结合实际控制流判断是否需要 early return 或提取 helper
